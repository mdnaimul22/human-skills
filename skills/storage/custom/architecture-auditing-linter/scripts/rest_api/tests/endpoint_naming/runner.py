...
from rest_api.endpoint_naming import EndpointNamingConvention
from cases import TEST_CASES

def run_tests():
    linter = EndpointNamingConvention()
    passed = 0
    ...
    for case in TEST_CASES:
        ...
        print(f"{case['id']:<25} | {case['expected_score']:<10.2f} | {actual_score:<10.2f} | {status}")
        if status == "❌ FAIL":
